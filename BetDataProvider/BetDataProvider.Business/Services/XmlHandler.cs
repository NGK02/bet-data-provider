...
namespace BetDataProvider.Business.Services
{
    public class XmlHandler : IXmlHandler
    {
        ...
        private HashSet<Odd> MergeAndUpdateOdds(HashSet<Odd> oldOdds, HashSet<Odd> newOdds)
        {
            ...
            updatedOdds.UnionWith(UpdateInactiveOdds(oldOdds));
        ...
        private HashSet<Match> UpdateInactiveMatches(HashSet<Match> inactiveMatches)
        {
            foreach (var inactiveMatch in inactiveMatches)
            {
                inactiveMatch.IsActive = false;
                inactiveMatch.Bets = UpdateInactiveBets(inactiveMatch.Bets);
            }
            return inactiveMatches;
        }

        private HashSet<Bet> UpdateInactiveBets(HashSet<Bet> inactiveBets)
        {
            foreach (var inactiveBet in inactiveBets)
            {
                inactiveBet.IsActive = false;
                inactiveBet.Odds = UpdateInactiveOdds(inactiveBet.Odds);
            }
            return inactiveBets;
        }

        private HashSet<Odd> UpdateInactiveOdds(HashSet<Odd> inactiveOdds)
        {
